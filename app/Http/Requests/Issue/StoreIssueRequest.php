...
class StoreIssueRequest extends FormRequest
{
    ...
    public function authorize(): bool
    {
        ...
        $parentIsSubIssue = Issue::query()
            ->where('id', '=', $parentId)
            ->whereNotNull('parent_id')
            ->exists();

        if (!$checkProjects && $checkIfUserTeamMember && !$parentIsSubIssue) {
            return true;
        }
        return false;
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'type' => ['required', 'string', Rule::in('task', 'bug', 'story')],
            ...
            'sprint_id' => ['nullable', Rule::exists('sprints', 'id')
                ->where(function ($query) use ($projectId) {
                    $query->where('project_id', $projectId);
                })],
            'project_id' => ['required'],
